...
describe('exercises (all the data passed to functions comes from data.js file, unless explicitly stated on the test)', () => {
    ...
    describe('basics', () => {
        ...
        xit('write a function that calculate the prices of the ', () => {
            const clients = [
        ...
        xit('given books collection, write one function that returns an array with writter names, another with titles and another with prices', () => {
            const get = currify((prop, object) => object[prop]);
            const getNames = books => books.map(({writter}) => writter);
            const getTitles = books => books.map(({title}) => title);

            expect(getNames(books)).to.deep.equal(books.map(get('writter')));
            expect(getTitles(books)).to.deep.equal(books.map(get('title')));
        });
